...
export function parseGallupExcel(buffer: Buffer): GallupExcelParseResult {
  ...
  try {
    workbook = XLSX.read(buffer, { type: "buffer", sheets: 0, sheetRows: DOCUMENT_LIMITS.spreadsheetRows + 1, cellFormula: false, cellHTML: false, cellStyles: false, bookVBA: false });
  } catch {
    result.errors.push("Failed to read Excel file. Please upload a valid Gallup export.");
    return result;
  }
  ...
  // Check original dimensions too: SheetJS truncates !ref when sheetRows is set.
  const originalRange = XLSX.utils.decode_range(sheet["!fullref"] || sheet["!ref"] || "A1");
  if (originalRange.e.r >= DOCUMENT_LIMITS.spreadsheetRows || originalRange.e.c >= DOCUMENT_LIMITS.spreadsheetColumns) {
    throw new DocumentParseError("LIMIT", "Excel exceeds the 1,020 row or 128 column limit");
  }
  let characters = 0;
  for (const [address, cell] of Object.entries(sheet)) {
    if (address.startsWith("!")) continue;
    const value = normalizeCell(cell?.v);
    characters += value.length;
    if (value.length > DOCUMENT_LIMITS.cellCharacters || characters > DOCUMENT_LIMITS.textCharacters) {
      throw new DocumentParseError("LIMIT", "Excel cell or total text exceeds the import limit");
    }
  }
  ...
  for (let r = dataStartRow; r <= range.e.r; r++) {
    ...
    for (const [colIdx, themeInfo] of layout.themeColumns) {
      ...
      const rank = /^\d+$/.test(val) ? Number(val) : NaN;
